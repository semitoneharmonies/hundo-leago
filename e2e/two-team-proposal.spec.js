...
test("two-team send keeps failed drafts, shows sending, then opens the created offer", async ({ page }) => {
  ...
  await expect(page.getByRole("group", { name: "Trade breakdown" }).getByRole("region")).toHaveCount(4);
  await expect(page.getByRole("button", { name: "Submit trade", exact: true })).toBeEnabled();
  ...
  await page.screenshot({ path: test.info().outputPath("two-team-review.png"), fullPage: true });
  ...
  await page.getByRole("button", { name: "Edit trade", exact: true }).click();
  ...
  await sending.evaluate(button => { button.click(); button.click(); });
  await expect.poll(() => page.evaluate(() => window.counterFixture.requests.filter(r => r.method === "POST" && !r.pathname.endsWith("/trades/preview")).length)).toBe(2);
  ...
  await expect(impact.getByText("$6.25", { exact: true })).toHaveCount(2);
