...
describe("authoritative team roster page", () => {
  it("keeps Candidate Card workflows and results out of the team roster", () => {
    const data = workspace();
    const httpClient = {
      request: vi.fn(async () => {
        throw new Error("The roster must not request draft navigation.");
      }),
      resourceUrl: vi.fn((value) => value),
    };
    renderWithProviders(
      <TeamRosterPage
        workspace={data}
    ...
    expect(screen.queryByText("Season Candidate Card")).toBeNull();
    expect(screen.queryByRole("link", { name: /Candidate Card/i })).toBeNull();
