...
export function createThreeTeamFixture({ status = "proposed", secondAccepted = false } = {}) {
  const ids = threeTeamIds, fixture = createCounterFixture({ status }), baseFetch = fixture.fetch;
  fixture.teams[0].currentManager.userId = "11111111-1111-4111-8111-000000000002";
  ...
  const own = proposal => proposal.participants.find(p => p.teamId === ids.thirdTeam);
  fixture.fetch = async (url, options = {}) => {
    ...
    if (pathname.includes("/trades") && (method === "POST" || pathname.endsWith("/acceptance-preview"))) {
      ...
      if (pathname.endsWith("/acknowledge")) { own(proposal).acknowledgedAtMs = 2; return envelope({ code: "TRADE_ACKNOWLEDGED" }); }
      if (pathname.endsWith("/accept")) { own(proposal).decision = "accepted"; own(proposal).respondedAtMs = 2; proposal.version++;
      ...
      if (pathname.endsWith("/decline")) { own(proposal).decision = "declined"; own(proposal).respondedAtMs = 2; proposal.storageStatus = "declined"; proposal.status = "Rejected"; proposal.version++; return envelope({ code: "TRADE_PROPOSAL_REJECTED" }); }
