...
/* =========================================================
   Team colors (Session 3)
   - tweak anytime; just keep keys stable
   ========================================================= */
const TEAM_COLORS = {
...
const hexToRgba = (hex, a) => {
  ...
  if (h.length !== 6) return `rgba(96,165,250,${a})`; // fallback
...
/* =========================================================
   Sorters
   - numeric ones are ascending by default; direction flips later
   ========================================================= */
const sorters = {
  ...
  owner: (a, b) => String(a.ownerTeamName || "").localeCompare(String(b.ownerTeamName || "")),
...
export default function FreeAgentsPage({
  ...
  /* =========================================================
     ...
     ========================================================= */
  const [scope, setScope] = useState("FREE_AGENTS");
...
const [nhlScope, setNhlScope] = useState("ALL"); // ALL | e.g. "VAN"
  ...
  /* =========================================================
     Session 4: compare list (session-only)
     - clears when myTeamName changes
     ========================================================= */
  const [compareIds, setCompareIds] = useState(() => new Set());

  /* =========================================================
     Auction panel state (kept)
     ========================================================= */
  ...
  /* =========================================================
     Auction: debounced player search (panel)
     ========================================================= */
  useEffect(() => {
  ...
  /* =========================================================
     Auction: prefill when clicking $ on a free-agent row
     ========================================================= */
  useEffect(() => {
  ...
  /* =========================================================
     ...
     ownerByPlayerId: Map<number, string>
     - determine "owned" from team.roster + team.ir (owned players)
     ========================================================= */
  const ownerByPlayerId = useMemo(() => {
    const by = new Map();

    const scanArr = (teamName, arr) => {
      for (const p of Array.isArray(arr) ? arr : []) {
        const pid =
          ...
          normId(p?.key);
        if (!pid) continue;
        // first win keeps ownership stable if duplicates exist
        if (!by.has(pid)) by.set(pid, teamName);
      }
    };
    ...
    for (const t of Array.isArray(teams) ? teams : []) {
      ...
      // Owned = roster + IR (adjust later if you want different semantics)
      scanArr(teamName, t?.roster);
      scanArr(teamName, t?.ir);
    }
  ...
  /* =========================================================
     Build unified players list (Session 1)
     rowsAll = stats players enriched with ownership
     ========================================================= */
  const rowsAll = useMemo(() => {
    ...
    for (const pid of ids) {
      ...
      const ownerTeamName = ownerByPlayerId.get(pid) || null;
      const isFreeAgent = !ownerTeamName;

      const ownerColor = isFreeAgent
        ? FREE_AGENT_COLOR
        : TEAM_COLORS[ownerTeamName] || "#60a5fa";
    ...
    return out;
  }, [statsByPlayerId, playerApi, ownerByPlayerId]);
  ...
  const applyScope = (arr) => {
    ...
    if (parsed.type === "MY_TEAM") return arr.filter((p) => p.ownerTeamName === myTeamName);
    if (parsed.type === "FA_PLUS_MY_TEAM")
      return arr.filter((p) => p.isFreeAgent || p.ownerTeamName === myTeamName);
    if (parsed.type === "ALL_EXCEPT_MINE")
      return arr.filter((p) => p.isFreeAgent || p.ownerTeamName !== myTeamName);
    if (parsed.type === "TEAM") return arr.filter((p) => p.ownerTeamName === parsed.teamName);
    if (parsed.type === "COMPARE") return arr.filter((p) => compareIds.has(p.playerId));
    if (parsed.type === "NHL") {
      if (parsed.teamAbbrev === "ALL") return arr;
      return arr.filter((p) => String(p.teamAbbrev || "").toUpperCase() === parsed.teamAbbrev);
    }
  ...
  /* =========================================================
     Filter + sort + paginate (Sessions 2/5)
     ========================================================= */
  const filteredRows = useMemo(() => {
    ...
    // position
    if (posFilter !== "ALL") {
      out = out.filter((r) => r.posGroup === posFilter);
    }

    // min GP
    if (Number(minGp) > 0) {
      out = out.filter((r) => (Number(r.gamesPlayed) || 0) >= Number(minGp));
    }
    ...
    if (q) {
      out = out.filter((r) => {
        const nm = normName(r.fullName);
        const team = normName(r.teamAbbrev);
        const owner = normName(r.ownerTeamName || "");
        return nm.includes(q) || team.includes(q) || owner.includes(q);
      });
    }
  ...
  useEffect(() => {
    // whenever filters/scope change, snap back to first page
    setVisibleCount(50);
  }, [scope, posFilter, minGp, listSearch, sortKey, sortDir]);
  ...
  const toggleSort = (key) => {
    if (sortKey === key) {
      ...
      // default desc for stats columns
      let next = "desc";
      if (key === "name" || key === "team" || key === "age" || key === "owner") next = "asc";
      setSortDir(next);
    }
  };
  ...
  /* =========================================================
     Auction derived values (kept)
     ========================================================= */
  const bids = Array.isArray(freeAgents) ? freeAgents : [];
  const activeBids = bids.filter((b) => !b?.resolved);
  const myBids = isManager ? bids.filter((b) => b.team === myTeamName && !b?.resolved) : [];
      ...
      <div style={{ width: "100%", maxWidth: "100%", minWidth: 0 }}>
        ...
        >
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ color: MUTED, fontSize: "0.9rem", whiteSpace: "nowrap" }}>Scope:</div>
            <select
  value={scope}
...
>
  ...
  {isManager && <option value="MY_TEAM">My Team</option>}
  {isManager && <option value="FA_PLUS_MY_TEAM">Free Agents + My Team</option>}
  {isManager && <option value="ALL_EXCEPT_MINE">All Except Mine</option>}

  {Array.isArray(teams) &&
    teams.map((t) => (
      <option key={t.name} value={`TEAM:${t.name}`}>
        Team: {t.name}
      </option>
    ))}

  <option value="COMPARE">{`Compare (${compareIds.size})`}</option>
  <option value="NHL:ALL">NHL: All Teams</option>
{NHL_TEAMS.map((abbrev) => (
  <option key={abbrev} value={`NHL:${abbrev}`}>
    NHL: {abbrev}
  </option>
))}
            ...
            {scope === "COMPARE" && compareIds.size > 0 && (
              <button
          ...
          {/* Position */}
          <div style={{ minWidth: 0 }}>
            <div style={{ color: MUTED, fontSize: "0.9rem", whiteSpace: "nowrap" }}>Position:</div>
            <select
              value={posFilter}
          ...
          {/* Min GP */}
          <div style={{ minWidth: 0 }}>
            <div style={{ color: MUTED, fontSize: "0.9rem", whiteSpace: "nowrap" }}>Min GP:</div>
            <select
              value={minGp}
      ...
      {/* Header row */}
      {isMobile ? (
        // MOBILE: headers matching the bottom stats line grid
        <div
          style={{
            marginTop: 10,
            padding: "0 10px 6px",
        ...
        <div
          style={{
            ...
            display: "grid",
            gridTemplateColumns:
  "54px minmax(105px,1fr) 52px 82px 44px 58px 58px 58px 66px 66px 58px 140px",
gap: 6,
      ...
      <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 8 }}>
        {visibleRows.map((r) => {
          ...
          const isCompared = compareIds.has(r.playerId);
          ...
          return (
            ...
            >
              ...
              {/* Team gradient for rostered players (FAs = no gradient) */}
{!r.isFreeAgent && (
  <div
    style={{
      position: "absolute",
      ...
      background: `linear-gradient(90deg,
        rgba(2,6,23,0) 0%,
        rgba(2,6,23,0) 45%,
        ${hexToRgba(r.ownerColor, 0.14)} 72%,
        ${hexToRgba(r.ownerColor, 0.26)} 100%
      )`,
      pointerEvents: "none",
    }}
  />
                  ...
                  >
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      >
                        ...
                        {/* owner / FA tag */}
                        <div
                          title={r.isFreeAgent ? "Free Agent" : `Owned by ${r.ownerTeamName}`}
                          style={{
                            flex: "0 1 auto",
                            padding: "3px 6px",
                            borderRadius: 999,
                            border: `1px solid ${r.ownerColor}`,
                            background: "rgba(2,6,23,0.55)",
                            color: r.isFreeAgent ? "#fde68a" : TEXT,
                            fontWeight: 900,
                            fontSize: "0.66rem",
                            whiteSpace: "nowrap",
                            maxWidth: 108,
                        ...
                        >
                          {r.isFreeAgent ? "FA" : r.ownerTeamName}
                        </div>
                        ...
                        <button
                          ...
                          style={{
                            ...
                            borderRadius: 9,
                            border: "1px solid #334155",
                            color: isCompared ? "#fde68a" : MUTED,
background: isCompared ? "rgba(250,204,21,0.18)" : "#0b1220",
                            cursor: "pointer",
                            fontWeight: 900,
                            flex: "0 0 auto",
                          }}
...
gap: 6,
        ...
        {statsReady && filteredRows.length === 0 && (
          <div style={{ color: MUTED, padding: "10px 2px" }}>
            No players found for this scope/filters (or stats not loaded yet).
          </div>
        )}
      </div>
  ...
  /* =========================================================
     STOP POINT (chunk 1)
     Next chunk will include:
     - RightColumn (team card + auction panel)
     - Page wrapper + top bar + 2-column layout
     ========================================================= */

  // (continue)
  const RightColumn = (
    <div>
      {/* Quick reference (your team card) */}
      <div
        style={{
      ...
      >
        ...
        >
          ...
          >
            <div>
              <strong style={{ color: TEXT }}>{myCounts.rosterSize}</strong> / {maxRosterSize}{" "}
              rostered · F: <strong style={{ color: TEXT }}>{myCounts.F}</strong> · D:{" "}
          ...
          {/* Compare hint */}
          <div style={{ marginTop: 4, color: MUTED, fontSize: "0.82rem" }}>
            Compare list: <strong style={{ color: TEXT }}>{compareIds.size}</strong>
            {scope === "COMPARE" ? (
              <span style={{ color: "#7dd3fc", fontWeight: 900 }}> · Viewing Compare</span>
            ) : null}
          </div>
        </div>
      </div>
      ...
      >
        <div style={{ color: TEXT, fontWeight: 900, marginBottom: 10 }}>Start Auction</div>
        ...
        >
          <div style={{ color: MUTED, fontSize: "0.85rem" }}>
            ...
            <strong style={{ color: TEXT }}>
              {nextSunday.toLocaleString(undefined, {
                weekday: "short",
                month: "short",
                day: "numeric",
                hour: "numeric",
          ...
          <div style={{ color: MUTED, fontSize: "0.82rem", marginTop: 4 }}>
            Time remaining:{" "}
            <strong style={{ color: TEXT }}>
              {Math.floor(timeRemainingMs / 3600000)}h{" "}
              {Math.floor((timeRemainingMs % 3600000) / 60000)}m{" "}
              {Math.floor((timeRemainingMs % 60000) / 1000)}s
            </strong>
          </div>
          <div style={{ color: MUTED, fontSize: "0.80rem", marginTop: 4 }}>
            New-auction cutoff:{" "}
            <strong style={{ color: TEXT }}>
              {auctionCutoff.toLocaleString(undefined, {
                weekday: "short",
                hour: "numeric",
                minute: "2-digit",
              })}
        ...
        {isManager ? (
          ...
          <div style={{ color: MUTED, fontSize: "0.9rem", padding: "6px 2px" }}>
  ...
  Showing{" "}
  <strong style={{ color: TEXT }}>
    {Math.min(visibleCount, filteredRows.length)}
  </strong>{" "}
  of <strong style={{ color: TEXT }}>{filteredRows.length}</strong> players
</>
