...
This work plan defines the next contained coding step. It does not begin that step merely by existing.

Grae delegated the technical planning decisions and approved adoption of this work plan on 2026-07-18. Coding begins only when Grae requests execution.
...
```json
{
  "scripts": {
    ...
    "test": "node --test",
    "test:characterization": "node --test test/characterization",
...
```powershell
Get-Content ..\hundo-leago\docs\06-work-plans\ACTIVE_WORK_PLAN.md
Select-String -Path ..\hundo-leago\docs\06-work-plans\ACTIVE_WORK_PLAN.md -Pattern '^`APPROVED`$','^`READY TO START`$','BR-00','Stop Conditions'
```

Expected result:

* the plan is approved and ready;
* only BR-00 is active;
