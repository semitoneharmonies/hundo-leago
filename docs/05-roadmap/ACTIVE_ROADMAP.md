...
Action: Publish the exact-nine RC-STG-006O23D V7 dynamic-loader read-only continuation as one literal non-merge child of retired V6 authority 3c87d50e613e9f3292ac5808a5dcbabd7aa29108, execute the distinct prebinding diagnostic loader once, bind only its exact observed at plus bindingObservationProjectionSha256, then make exactly three phase-scoped production-loader submissions (PRE, POST, and FINAL), each its phase's sole one-shot, and execute only the frozen PRE/held-PRE/arm/sealed-npm/POST/activation/held-POST/FINAL/postflight/cleanup sequence defined below
